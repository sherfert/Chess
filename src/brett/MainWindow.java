...
public class MainWindow extends JFrame {
    ...
    public class AbstandPanel extends JPanel {
        ...
        /**
         * @param horizontal horizontaler Abstand in Pixeln
         * @param vertikal   vertikaler Abstand in Pixeln
         */
    ...
    public class Schwierigkeitsregler extends JPanel {
        ...
        /**
         * @param enabled ob der Regler an sein soll
         */
        @Override
    ...
    private void menuezeileErzeugen(JFrame fenster) {
        ...
        eintrag = new JMenuItem("Spiel speichern");
        eintrag.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                speichern();
            }
        ...
        eintrag = new JMenuItem("Spiel laden");
        eintrag.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                laden();
            }
    ...
    /**
     * Öffnet einen Dialog, um das Spiel zu speicher.
     */
    private void speichern() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            try (FileWriter fw = new FileWriter(fc.getSelectedFile())) {
                Spieler weiss = brett.getWeiss();
                Spieler schwarz = brett.getSchwarz();
                SchachUhrThread weissTimer = brett.getWeissTimer();
                SchachUhrThread schwarzTimer = brett.getSchwarzTimer();


                String textToSave = spielerSpeicherString(weiss, weissTimer) + "\n"
                        + spielerSpeicherString(schwarz, schwarzTimer) + "\n"
                        + textArea.getText();
                fw.append(textToSave);
            } catch (IOException e) {
                // ignore
                e.printStackTrace();
            }
    ...
    /**
     * Einen String der den Spieler (und ggf. die Zeit seines Timers) beschreibt.
     */
    private String spielerSpeicherString(Spieler spieler, SchachUhrThread timer) {
        if (spieler instanceof MyPCSpieler pcSpieler) {
            return pcSpieler.getSchwierigkeit() + (pcSpieler.isAgressiv() ? "A" : " ");
        } else if (timer == null) {
            return "M";
        } else {
            return "M" + timer.getMillisekunden();
        }
    }

    /**
     * Lädt einen Spieler aus einem String der von {@link spielerSpeicherString} generiert wurde.
     */
    private Spieler spielerLaden(String string) {
        switch (string.charAt(0)) {
            case 'M' -> {
                return new Spieler(brett);
            }
            default -> {
                boolean agressiv = string.charAt(1) == 'A';
                int schwierigkeit = string.charAt(0) - '0';
                return new MyPCSpieler(brett, schwierigkeit, agressiv);
            }
        }
    }

    /**
     * Lädt einen Timer aus einem String der von {@link spielerSpeicherString} generiert wurde.
     *
     * @return die millisekunden für den Timer.
     */
    private long timerLaden(String string) {
        switch (string.charAt(0)) {
            case 'M' -> {
                if (string.length() > 1) {
    ...
    /**
     * Öffnet einen Dialog, um das Spiel zu laden.
     */
    private void laden() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                // Start a new game
                Spieler spieler0 = spielerLaden(lines.get(0));
                Spieler spieler1 = spielerLaden(lines.get(1));
                SchachUhrThread timer0 = null;
                SchachUhrThread timer1 = null;

                if (spieler0 instanceof MyPCSpieler || spieler1 instanceof MyPCSpieler) {
                    denkFortschritt = new JProgressBar(0, 100);
                    zeitPanel.add(denkFortschritt);
                } else {
                    // Zeitspiel is möglich
                    long timerWeiss = timerLaden(lines.get(0));
                    long timerSchwarz = timerLaden(lines.get(1));

                    if (timerWeiss > 0 && timerSchwarz > 0) {
                        boolean istWeissDran = brett.aktuelleFarbe() == Farbe.WEISS;
                        initialisiereZeitAnzeigen();
                        timer0 = new SchachUhrThread(brett, timerWeiss, Farbe.WEISS, zeitAnzeigen[0], istWeissDran);
                        timer1 = new SchachUhrThread(brett, timerSchwarz, Farbe.SCHWARZ, zeitAnzeigen[1], !istWeissDran);
                    }
                }

                brett.reset(spieler0, spieler1, timer0, timer1);
                pack();

                List<String> moves = lines.stream().skip(2).toList();

                // Make each move from the file
                var farbe = Farbe.WEISS;
                for (String move : moves) {
                    var zug = brett.new Zug(move, farbe);
                    zug.fuehreAus(true);
                    farbe = farbe.andereFarbe();
                }
        ...
        if (playType.equals("mm")) {
            ...
            if (zeitSpiel) {
                ...
                brett.reset(
                        new Spieler(brett),
                        new Spieler(brett),
                        new SchachUhrThread(brett,
                                (stunden * 60 + minuten) * 60000,
                                Farbe.WEISS, zeitAnzeigen[0], true),
                        new SchachUhrThread(brett,
                                (stunden * 60 + minuten) * 60000,
                                Farbe.SCHWARZ, zeitAnzeigen[1], false));
            } else {
    ...
    /**
     * Initialisiert die grafischen anzeigen für ein Zeitspiel.
     */
    private void initialisiereZeitAnzeigen() {
     ...
     *
     * @param name der Name der abzuspielenden Sounddatei
     */
     ...
     *
     * @param text den zu schreibenden Text
     */
